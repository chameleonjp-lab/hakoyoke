...
| 問題生成規則           | `client/src/game/puzzles.ts`      | Stage 1〜4全44問＋Stage 5全12問＋Stage 6全8問＋Stage 7全12問は手設計、残りは決定的生成。`pnpm puzzles:write` |
...
Stage 1のWave 1はAREAを使わず、VOIDを捕獲せずに通す読みと列移動を学ぶ3問です。Wave 2は先頭のVEILからAREAを作り、3×3範囲の外側だけを手動捕獲する3問です。Wave 3–4は分岐・交差・角・編み込み・往復・周回という6つの手設計ルートで、AREAを覚えた後も次の安全マスを読む局面を置いています。Stage 2のWave 1–2は、千鳥・分岐・単線・編み込み・ゲート・往復という6つの手設計ルートでAREAを使わずにルートを読み、Wave 3–4はAREAを3回連鎖させながら外側の列を手動捕獲する6つの手設計局面です。Stage 3のWave 1–2は、幅5列への拡張に合わせて、中央分断・外側の分岐・長い編み込み・帰還という6つの手設計ルートを置き、Wave 3–4は幅5列のAREA連鎖と外周捕獲を組み合わせた6つの手設計局面です。Stage 4のWave 1–2は奥行7列、Wave 3–4は奥行8列で、4回のAREA発射、外周の間隔、遅延、切り返しを読む8つの手設計局面です。Stage 5のWave 1–2は幅6列のAREA帯と外周レーンを入れ替え、Wave 3–4は奥行7列の長いAREA連鎖と遅延・切り返しを読む12の手設計局面です。Stage 6のWave 1–2は奥行8列、Wave 3–4は奥行9列で、4〜5回のAREA発射と外周の遅延・反転・帰還を読む8つの手設計局面です。Stage 7のWave 1–2は幅7列・奥行7列、Wave 3–4は幅7列・奥行8列で、4回のAREA発射と外周のリボン・反転・パルス・ゲート・切り返し・帰還を読む12の手設計局面です。この序盤76問は`puzzles.ts`内の`HAND_AUTHORED_DESIGNS`を正本とし、Stage 8以降は既存の制約付き決定的生成を使います。保護・連鎖・持ち越しAREAは後続Stageで段階的に扱います。
...
- 序盤76問の手設計導入（Stage 1全12問: AREAなし→AREA導入→ルート読解、Stage 2全12問: ルート読解→AREA連鎖、Stage 3全12問: 幅5列のルート読解→AREA連鎖、Stage 4全8問: 奥行7〜8列の長いAREA連鎖、Stage 5全12問: 幅6列と奥行7列の長いAREA連鎖、Stage 6全8問: 奥行8〜9列の長いAREA連鎖、Stage 7全12問: 幅7列・奥行7〜8列のAREA連鎖）
