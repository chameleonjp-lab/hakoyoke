...
test("CREATEで問題セルを変更し、ローカル保存できる", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: /CAMPAIGN/ }).click();
  await page.getByRole("button", { name: /CREATE BUILD A CUSTOM ORDEAL/ }).click();
  await page.getByRole("button", { name: /CONFIGURE/ }).click();
  await expect(page.getByText("CREATE // LOCAL ARCHIVE")).toBeVisible();
  await page.locator(".editor-cell").first().click();
  await page.getByRole("button", { name: /SAVE/ }).click();
  await expect(page.getByText(/CUSTOM ARCHIVEへ保存しました。保存数:/)).toBeVisible();
  await expect(page.getByRole("button", { name: /MIRROR/ })).toBeVisible();
  await expect(page.getByRole("button", { name: /IMPORT/ })).toBeVisible();
});

test("TUTORIALとDUELをそれぞれ開始でき、専用状態がHUDへ出る", async ({ page }) => {
  await page.goto("/");
...
test("Campaignの状態と設定をlocalStorageへ保存し、Campaign開始時に復帰できる", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: /SETTINGS/ }).click();
  await page.locator("select").first().selectOption("LOW");
  await page.locator("select").nth(1).selectOption("OFF");
  await expect.poll(() => page.evaluate(() => localStorage.getItem("cubic-ordeal-quality"))).toBe("LOW");
  await expect.poll(() => page.evaluate(() => localStorage.getItem("cubic-ordeal-audio"))).toBe("OFF");
...
test("プレイヤーが足場外へ出ると即座にゲームオーバーになる", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "FALL INTO VOID" })).toBeVisible();
});

test("スマートフォン縦画面でタップ地点に移動キーが出現し、画面上方向へ前進できる", async ({ page }) => {
