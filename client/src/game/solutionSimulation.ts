...
import { DIFFICULTIES, type AreaMark, type CubeState, type PuzzleDescriptor, type SolutionStep } from "./types";
...
const ACTION_PRIORITY: Record<SolutionStep["action"], number> = { capture: 0, mark: 1, area: 2 };
...
export function simulatePuzzleSolution(puzzle: PuzzleDescriptor): SolutionSimulationResult {
  ...
  for (const step of steps) {
    ...
    if (step.action === "mark") {
      if (step.x === undefined || step.z === undefined) return failure("mark without position");
      ...
      const available = elapsedRotations * cycleSeconds + config.captureSeconds + config.settleSeconds;
      if (distance > available * config.playerSpeed + 0.35) playerReachable = false;
    ...
    if (step.action === "capture") {
      ...
      if (target.type === "veil") {
        const next: AreaMark = { id: `area-${target.id}`, x: target.x, z: target.z, armed: true };
  ...
  if (!playerReachable) return failure("player cannot reach a scheduled MARK", measuredRolls, false, voidCaptured, areaUses, consumedAreaAnchors, regeneratedAreaAnchors);
  if (voidCaptured) return failure("solution captures VOID", measuredRolls, true, voidCaptured, areaUses, consumedAreaAnchors, regeneratedAreaAnchors);
  if (remaining) return failure("solution leaves required cubes", measuredRolls, true, voidCaptured, areaUses, consumedAreaAnchors, regeneratedAreaAnchors);
  if (measuredRolls !== puzzle.requiredRolls) return failure("requiredRolls differs from replay", measuredRolls, true, voidCaptured, areaUses, consumedAreaAnchors, regeneratedAreaAnchors);
  return {
    valid: true,
    reason: "ok",
    requiredCaptured: cubes.filter((cube) => cube.type !== "void" && cube.captured).length,
    voidCaptured,
    measuredRolls,
    playerReachable,
    areaUses,
...
export function deriveDirectSolution(puzzle: Pick<PuzzleDescriptor, "id" | "width" | "depth" | "layout">): SolutionStep[] {
  ...
  return puzzle.layout
    ...
    .flatMap((cube) => {
      ...
      return [
        { rotation: Math.max(0, rotation - 1), action: "mark" as const, x: cube.x, z: targetZ },
...
function compareSteps(a: SolutionStep, b: SolutionStep): number {
  return a.rotation - b.rotation || ACTION_PRIORITY[a.action] - ACTION_PRIORITY[b.action];
}
...
function failure(
  ...
  return { valid: false, reason, requiredCaptured: 0, voidCaptured, measuredRolls, playerReachable, areaUses, consumedAreaAnchors, regeneratedAreaAnchors };
}
