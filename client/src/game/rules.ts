...
export function areaAnchorBlocksMark(areas: AreaMark[], position: GridPosition): boolean { return areas.some((area) => area.x === position.x && area.z === position.z); }
...
export function unresolvedCubeCount(cubes: CubeState[]): number { return cubes.filter((cube) => !cube.captured && !cube.falling).length; }
