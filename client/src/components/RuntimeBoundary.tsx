...
> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo): void {
    console.error(
      "CUBIC ORDEAL runtime boundary captured a renderer error",
      error,
      info
    );
  }
  ...
  render() {
    ...
    return (
      <section className="runtime-failure" role="alert" aria-live="assertive">
        <span>OBSERVATORY // RUNTIME ALERT</span>
        <h2>描画システムを再接続できません</h2>
        <p>
          ゲーム進行データには変更を加えていません。再試行しても解決しない場合は、画質を下げるか再読み込みしてください。
        </p>
        <button
        ...
        >
          RETRY RUNTIME <small>SAFE RELOAD</small>
        </button>
      </section>
    );
  }
