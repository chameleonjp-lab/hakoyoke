...
`/manus-storage/*`はExpressの検査・互換ルートです。明示allowlistにある公開資産だけを受け付け、Forge資格情報がある環境ではcredential-freeなHTTPS署名URLへ`307`、ない環境では機密情報を含まない`503`を返します。解決にはtimeoutとrate limitを設け、ゲームのタイトル表示や3D描画をこのルートへ接続しないことを現行仕様とします。
